...
CBrush* CMyButton::MakeBrush (int iState)
	{
	COLORREF clr1 = GetColor (iState, 0);
	COLORREF clr2 = clr1;

	CPalette* pPal = (m_pPal ? m_pPal : (m_pExternalPal ? m_pExternalPal : NULL));
	if (pPal)
		clr2 = PALETTEINDEX (pPal->GetNearestPaletteIndex (clr1));

	return (CBrush*) new CBrush (clr2);
	}
...
void CMyButton::DrawButtonShape (CDC* pDC, CRect& rect, int iState)
	{
	if (m_pData->m_bLinkType)
		{
		CCdmenuDlg* pDlg = (CCdmenuDlg*)GetParent ();

		CRect cRect;
		GetWindowRect(cRect);
		pDlg->ScreenToClient (cRect);
		CPoint ptOffset (-cRect.left,-cRect.top);

		pDlg->PaintBitmapBackground(pDC, rect, ptOffset);
		return;
		}
	...
	else if (m_pExternalPal && !m_pBmpPush)
		{
		pOldPal = pDC->SelectPalette(m_pExternalPal, TRUE);
		pDC->RealizePalette();
		}
...
void CMyButton::DrawButtonText (CDC* pDC, CRect& rect, int iState)
	{
	CPalette* pOldPal = NULL;
	COLORREF clr;
   ...
   pDC->SetBkMode(TRANSPARENT);
	...
	if (m_pData->m_bLinkType)
		{
		clr = (iState ? m_pData->m_clrHilight : m_pData->m_sdBtn.m_clrFont);
		}
	else
		{
...
//	DrawIt (pDC, rect, (m_bPushed ? 2 : (m_bHilight ? 1 : 0)));
	...
	DrawButtonShape (pDC, rect, (m_bPushed ? 2 : (m_bHilight ? 1 : 0)));
	DrawButtonText (pDC, rect, (m_bPushed ? 2 : (m_bHilight ? 1 : 0)));
	}
